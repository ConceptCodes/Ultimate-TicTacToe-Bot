...
import signal
import copy
#from old import Player2o
from backupfinal import Player2
...
class Random_Player:
    ...
    def move(self, board, old_move, flag):
        ...
        # cells = Player2().move(board, old_move, flag)
        # print cells
        # print cells
...
class Board:
    ...
    def print_board(self):
        ...
        print '==============Block State=============='  # Block is the whole(super) board
        for i in range(4):
            for j in range(4):
    ...
    def find_valid_move_cells(self, old_move):
        ...
        if old_move != (-1, -1) and self.block_status[allowed_block[0]][allowed_block[1]] == '-':
            ...
            # print'in simulator, bhar chuka hai'
            for i in range(16):
                for j in range(16):
                    if self.board_status[i][j] == '-' and self.block_status[i / 4][j / 4] == '-':
                        allowed_cells.append((i, j))
        # print 'final wala allowed_cells'
        # print allowed_cells
        return allowed_cells

    def find_terminal_state(self):
        # checks if the game is over(won or drawn) and returns the player who have won the game or the player who has
        #  higher blocks in case of a draw
    ...
    def check_valid_move(self, old_move, new_move):
        ...
        if (len(old_move) != 2) or (len(new_move) != 2):
            # print 'check1'
            return False
        if (type(old_move[0]) is not int) or (type(old_move[1]) is not int) or (type(new_move[0]) is not int) or (
                    type(new_move[1]) is not int):
            # print 'check2'
            return False
    ...
    def update(self, old_move, new_move, ply):
        ...
        if (self.check_valid_move(old_move, new_move)) == False:
            return 'UNSUCCESSFUL'
        # UPDATING THE BOARD WITH THE INPUT TOKEN
...
def gameplay(obj1, obj2):  # game simulator
    ...
    while (1):
        ...
        try:
            ...
            # print 'I am P1'
            p1_move = Player1().move(game_board, old_move, fl1)
            # print 'p1 got'
        ...
        try:
            p2_move = Player2().move(game_board, old_move, fl2)
        except TimedOutExc:
            WINNER = 'P1'
            MESSAGE = 'TIME OUT'
            pts1 = 16
